...
let parseJsonLinks = (res: Js.Json.t) : stateType => {
  links: field("links", list(parseListItem), res)
};

let parseJsonResponse = (res: Js.Json.t) : dataType => {
  data: field("data", parseJsonLinks, res)
};
...
let fetchLinks = () =>
  Js.Promise.(
    ...
    |> then_(jsonText =>
         resolve(parseJsonResponse(Js.Json.parseExn(jsonText)))
       )
  );
      ...
      <ul>
        (
          List.map(
            (link: Types.link) =>
              <li key=link.rel>
                <a href="#" onClick=(e => preventDefault(e, link))>
                  (str(link.rel))
                </a>
              </li>,
            self.state.links
